...
func (cli *hostClient) ConnectorCreate(secretFile string, options types.ConnectorCreateOptions) (string, error) {

	sc, err := cli.SiteConfigInspect(types.DefaultBridgeName)
	if err != nil {
		return "", fmt.Errorf("Unable to retrieve site config (need init?): %w", err)
	}
	...
	current.AddConnSslProfileToPath("/var/tmp/skupper", qdr.SslProfile{
		Name: profileName,
	})
